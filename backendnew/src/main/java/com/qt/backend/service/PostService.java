...
public class PostService {
    ...
    // public PostDto getPostByIdAndUserId(Long postId, Long userId) {
    ...
    // postDto.setIsLiked(likeRepository.findAnyLikeByPostIdAndUserId(postId,
    // userId));
    // postDto.setIsSaved(saveRepository.findAnySaveByPostIdAndUserId(postId,
    // userId));
    // return postDto;
    // }
    ...
    public List<PostDto> getFeedPostsForUser(Long userId, int page) {
    ...
    public List<PostDto> getReelsPostsForUser(Long userId, int page) {
        HashSet<String> followingIds = new HashSet<>(followsRepository.findFollowingByUserId(userId.toString())
                .stream().map(dto -> dto.getUserId()).toList());
